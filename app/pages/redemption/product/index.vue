  ...
  <div class="flex flex-col w-full">
        ...
        <div class="flex items-center gap-4">
          <div>
            <div class="flex items-center gap-2">
              <h1 class="text-xl font-medium text-neutral-800">Tukar Poin - Produk</h1>
            ...
            <p class="text-xs text-neutral-400 font-medium mt-0.5">
              <NuxtLink to="/" class="text-primary hover:underline">Home</NuxtLink> / Tukar Poin / Produk
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="flex flex-col gap-4 w-full">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between w-full mb-2 border-b border-transparent md:border-base-200 gap-3 md:gap-0">
        ...
        <!-- Bulk Action Button -->
        <div v-if="activeTab === 'processing' && canEdit('redemption.product')" class="flex items-center gap-4 pb-2.5 md:pb-2 pr-4">
          <!-- Select All Checkbox -->
        ...
        <template #body="{ isColumnVisible }">
          <tbody class="text-sm text-neutral-600">
            <tr v-for="(item, index) in items" :key="index" class="hover:bg-base-200/30 transition-colors border-b border-base-100 last:border-0">
              ...
              <td v-show="isColumnVisible('product')" class="border-r border-base-200">
                <div class="flex items-center gap-3">
                  ...
                  <div class="min-w-0">
                    <p class="font-medium text-neutral-800 truncate max-w-[160px]" :title="item.productDetails?.catalog?.name">{{ item.productDetails?.catalog?.name }}</p>
                    <p class="text-xs text-neutral-400 truncate max-w-[160px]">{{ item.productDetails?.catalog?.category?.name }}</p>
                  </div>
                </div>
              </td>
              <td v-show="isColumnVisible('recipient')" class="border-r border-base-200 max-w-[200px]">
                <p class="font-medium text-neutral-800 truncate" :title="item.productDetails?.name">{{ item.productDetails?.name }}</p>
                <p class="text-xs text-neutral-400 truncate" :title="item.productDetails?.address">{{ item.productDetails?.address }}</p>
              </td>
              ...
              <td v-show="isColumnVisible('shipping')" class="border-r border-base-200 max-w-[200px]">
                <template v-if="item.productDetails?.shipping">
                  <p class="font-medium text-neutral-800 uppercase text-xs">{{ item.productDetails.shipping.shipper }}</p>
                  <p class="text-xs text-neutral-400 truncate" :title="item.productDetails.shipping.trackingNumber">{{ item.productDetails.shipping.trackingNumber }}</p>
                </template>
              ...
              <td v-show="isColumnVisible('actions')" class="text-center">
                <div class="flex items-center justify-center gap-1">
                  ...
                  <template v-if="activeTab === 'pending' && canEdit('redemption.product')">
                    <button 
                      @click="openShippingModal(item)"
                  ...
                  <template v-else-if="activeTab === 'processing' && canEdit('redemption.product')">
                    <button 
                  ...
                  <template v-else>
                    <button class="btn btn-ghost btn-xs btn-circle" @click="openShippingDetail(item)">
    ...
    <!-- Modals -->
    <ModalProcessShipping ref="shippingModalRef" @success="fetchData({ page: 1 })" />
    ...
    <dialog ref="detailDialogRef" class="modal modal-bottom sm:modal-middle">
      <div class="modal-box max-w-md bg-white">
        <div class="flex items-center justify-between mb-6">
          <h3 class="font-bold text-lg text-neutral-800">Detail Pengiriman</h3>
        ...
        <div v-if="detailItem" class="space-y-4">
          <div class="flex items-center gap-3 p-3 bg-base-200/50 rounded-lg">
            <div class="w-12 h-12 rounded-lg overflow-hidden bg-base-200 shrink-0">
              <img v-if="detailItem.productDetails?.catalog?.image" :src="detailItem.productDetails.catalog.image" class="w-full h-full object-cover" />
            </div>
            <div class="min-w-0">
              <p class="font-medium text-sm text-neutral-800 truncate">{{ detailItem.productDetails?.catalog?.name }}</p>
          ...
          <div class="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <p class="font-medium text-neutral-800">{{ detailItem.productDetails?.name }}</p>
            </div>
            <div>
              <p class="text-neutral-400 text-xs mb-0.5">Telepon</p>
              <p class="font-medium text-neutral-800">{{ detailItem.productDetails?.phone }}</p>
            </div>
            <div class="col-span-2">
              <p class="text-neutral-400 text-xs mb-0.5">Alamat</p>
              <p class="font-medium text-neutral-800">{{ detailItem.productDetails?.address }}</p>
            </div>
            <div>
              <p class="text-neutral-400 text-xs mb-0.5">Kurir</p>
              <p class="font-medium text-neutral-800 uppercase">{{ detailItem.productDetails?.shipping?.shipper || '-' }}</p>
            </div>
            <div>
              <p class="text-neutral-400 text-xs mb-0.5">No. Resi</p>
              <p class="font-medium text-neutral-800">{{ detailItem.productDetails?.shipping?.trackingNumber || '-' }}</p>
            </div>
          </div>
        </div>
      </div>
...
import type { ProductRedemptionListItem, ProductRedemptionQueryParams } from '~/types/redemption'
...
useSeoMeta({
  title: 'Kawan Nusa | Tukar Poin - Produk',
})
...
const items = ref<ProductRedemptionListItem[]>([])
...
const openHistoryModal = (item: ProductRedemptionListItem) => {
...
// Modals
const shippingModalRef = ref<InstanceType<typeof ModalProcessShipping> | null>(null)
const detailDialogRef = ref<HTMLDialogElement | null>(null)
const detailItem = ref<ProductRedemptionListItem | null>(null)

const openShippingModal = (item: ProductRedemptionListItem) => {
  shippingModalRef.value?.open(item)
}

const openShippingDetail = (item: ProductRedemptionListItem) => {
...
const handleComplete = async (id: number) => {
  ...
  try {
    await redemptionService.completeProduct(id)
...
const handleBulkComplete = async () => {
  ...
  if (processingIds.length === 0) {
    const toast = useToast()
    toast.warning('Tidak ada item dengan status "Diproses" yang bisa diselesaikan. Proses pengiriman terlebih dahulu.')
    return
  }
  ...
  try {
    const promises = processingIds.map(id => 
      redemptionService.completeProduct(id)
...
const fetchData = async (queryParams: Partial<ProductRedemptionQueryParams> = {}) => {
  loading.value = true
  try {
    const params: ProductRedemptionQueryParams = {
    ...
    const response = await redemptionService.getProductRedemptions(params)
