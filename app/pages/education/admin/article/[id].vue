<template>
  <div class="flex flex-col w-full pb-12">
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            <div class="flex items-center gap-2">
              <h1 class="text-xl font-medium text-neutral-800">Edit Artikel</h1>
            </div>
    ...
    <div v-if="loadingDetail" class="w-full flex flex-col items-center justify-center min-h-[300px] gap-3">
      ...
      <span class="text-sm text-neutral-500 font-medium">Memuat data artikel...</span>
    </div>
      ...
      <div class="lg:col-span-2 space-y-6">
        <div class="card bg-white border border-base-200 shadow-xs p-6 rounded-xl space-y-4">
          ...
          <div>
            ...
            <div class="border border-base-300 rounded-lg overflow-hidden bg-white" :class="{ 'border-red-500': errors.content }">
              <QuillEditor 
                ref="quillEditor"
...
useSeoMeta({
  title: 'Kawan Nusa | Admin - Edit Artikel',
})
...
const fetchArticleDetail = async () => {
  ...
  try {
    const res = await educationService.getArticleById(articleId)
    ...
    toast.error('Terjadi kesalahan saat mengambil detail artikel')
    navigateTo('/education/admin/article')
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await educationService.updateArticle(articleId, formData)
    if (res.success) {
