<template>
  <div class="flex flex-col w-full pb-12">
        ...
        <div class="flex items-center gap-4">
          <div>
            <div class="flex items-center gap-2">
              <h1 class="text-xl font-medium text-neutral-800">Edit Video</h1>
            </div>
            <p class="text-xs text-neutral-400 font-medium mt-0.5">
              <NuxtLink to="/" class="text-primary hover:underline">Home</NuxtLink> / 
              <NuxtLink to="/education/admin/video" class="text-primary hover:underline">Konten Edukasi</NuxtLink> / 
              Edit
            </p>
          </div>
        </div>
    ...
    <!-- Loading State -->
    <div v-if="loadingDetail" class="w-full flex flex-col items-center justify-center min-h-[300px] gap-3">
      <Loader2 class="w-8 h-8 animate-spin text-primary" />
      <span class="text-sm text-neutral-500 font-medium">Memuat data video...</span>
    </div>
          ...
          <div>
            ...
            <div class="mt-1 flex flex-col items-center justify-center border-2 border-dashed border-base-300 rounded-xl p-4 bg-neutral-50/50 hover:bg-neutral-50 transition-colors cursor-pointer relative group">
              ...
              <template v-if="previewUrl">
                <img :src="previewUrl" class="max-h-48 rounded-lg object-cover w-full animate-fade-in" />
...
import { BookOpen, Upload, Loader2 } from 'lucide-vue-next'
import { z } from 'zod'
...
useSeoMeta({
  title: 'Kawan Nusa | Admin - Edit Video',
})
...
const fetchVideoDetail = async () => {
  ...
  try {
    const res = await educationService.getVideoById(videoId)
    ...
    toast.error('Terjadi kesalahan saat mengambil detail video')
    navigateTo('/education/admin/video')
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await educationService.updateVideo(videoId, formData)
    if (res.success) {
      toast.success('Video berhasil diperbarui')
      navigateTo('/education/admin/video')
    } else {
      toast.error(res.message || 'Gagal memperbarui video')
    }
  } catch (error) {
